...
def _get_models():
    PaisDestino = django_apps.get_model("system", "PaisDestino")
    TipoVisto = django_apps.get_model("system", "TipoVisto")
    Partner = django_apps.get_model("system", "Partner")
    StatusProcesso = django_apps.get_model("system", "StatusProcesso")
    FormularioVisto = django_apps.get_model("system", "FormularioVisto")
    PerguntaFormulario = django_apps.get_model("system", "PerguntaFormulario")
    OpcaoSelecao = django_apps.get_model("system", "OpcaoSelecao")
    EtapaCadastroCliente = django_apps.get_model("system", "EtapaCadastroCliente")
    CampoEtapaCliente = django_apps.get_model("system", "CampoEtapaCliente")
    User = get_user_model()
    ...
    for definicao in etapas_definitions:
        ...
        for campo_def in definicao.get("campos", []):
            defaults_campo = {
                ...
                "regra_exibicao": campo_def.get("regra_exibicao"),
            }
    ...
    for definicao in formularios_definitions:
        ...
        for pergunta_def in definicao.get("perguntas", []):
            defaults = {
                ...
                "regra_exibicao": pergunta_def.get("regra_exibicao"),
            }
...
def ensure_initial_domain_data(sender, **kwargs):
    ...
    try:
        with transaction.atomic():
            ...
            if formularios_definitions and not tipos_visto_por_nome:
                tipos_visto_por_nome = {
                    tipo.nome: tipo
                    for tipo in TipoVisto.objects.select_related("pais_destino").all()
                }
...
def _criar_registros_financeiros_para_viagem(viagem):
    ...
    Financeiro = django_apps.get_model("system", "Financeiro")
...
def _registrar_signals_financeiro():
    ...
    Viagem = django_apps.get_model("system", "Viagem")
